...
When Figma lacks a state, infer it from Figma's other families and name the node it came from (a missing hover on the secondary button comes from the hover on the primary button, for example). Mark it `inferred`.

Invent a value only when Figma has no source for it at all, and list it in the map under **Built without Figma**.

A color that fails WCAG AA ships as drawn: list the pair under **Questions for design** instead of adjusting the color.
...
- Side gutter: if page artboards show consistent side padding, use it; otherwise `1.5rem` mobile, `6rem` desktop (the kit's own placeholder values, see **The contract** in `foundation.md`). Tokens: `--padding-global` (mobile/base) and `--padding-global-lg` (the `lg` breakpoint up): the kit's `helpers.css` container already reads these two names, so change their values, don't add a differently-named token.
...
1. **Infer from Figma first.** Find the same state on another family (the primary button's hover for a missing secondary hover, the input's focus for a missing textarea focus) and carry over what changes: the color pair, the border, the underline. Cite the node in the map. Mark it `inferred`.
...
Text on a drawn or inferred color that falls under 4.5:1 ships as drawn and goes under **Questions for design**. Text on an invented color that falls under 4.5:1 is an invention error: pick the other direction of the shift, and list it under **Built without Figma**.
...
With no ring anywhere in the file, write one, list it under **Built without Figma**, and use `outline: 2px solid var(--color-ink); outline-offset: 2px;` on `:focus-visible`.
