...
Candidate background/foreground combinations, written into `<theme>/kit.config.json`'s `grounds` array once confirmed: see `SKILL.md` › **Grounds**.

| Seen on | Proposed name | Token | Light foreground | Confirmed |
| --- | --- | --- | --- | --- |
| Home hero, footer | dark | --color-ink | yes | no |
...
- `## Token mapping` (phase 1): the final Figma name → CSS token table as written, with `derived` and `inferred` marks. Every required token name (`foundation.md` › **Required tokens**) has a row.
...
- `## Contrast failures (built as drawn)` (phase 1, extended by later phases): every pair in `contrast-pairs.json` marked `"asDrawn": true`. One row per pair: foreground token and value, background token and value, ratio, required ratio, and the Figma node that draws it. Empty when every drawn pair passes.
- `## Components` (phase 2): component → node → CSS file.
- `## Block inventory` (phase 4): section → block name → source node → preview image → status.
- `## Template audit` (phase 5): template → exists → themed → action.
