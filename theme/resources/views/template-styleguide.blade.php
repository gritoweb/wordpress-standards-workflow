...
--}}
{{-- Internal reference page, driven by the live tokens in
     resources/css/global/variables.css through StyleGuide.php — no
     hand-kept palette or specimen list to drift out of sync. Dev-only: see
     _docs/launch-list.md before shipping a site.

     One section per design system board, in the same order on every
     project: typography, color, spacing, buttons, forms, components. The
     figma-design-system phases append their specimens inside the matching
     section and never add a seventh. --}}
@extends('layouts.app')

@section('content')
  <style>
    /* Styleguide-only layout, scoped to .styleguide. The specimens use the
             theme's own tokens and classes. */
    .styleguide .sg-header {
      padding-block: 6rem calc(6rem / 2);
    ...
    /* min(<fixed>, 100%) keeps the intended track width and lets it
             collapse when the container is narrower. */
    .styleguide .sg-grid {
      display: grid;
      gap: 1.5rem;
    ...
    /* A row of specimens that keep their Figma width and scroll instead of
             squeezing on a narrow screen. */
  ...
  <div class="styleguide">
    <div class="container">
      ...
      {{-- Dev-only: see _docs/launch-list.md before shipping a site. One section per
                 design system board, in the same order on every project: typography,
                 color, spacing, buttons, forms, components. The figma-design-system
                 phases append their specimens inside the matching section and never add
                 a seventh. Colors and type come from variables.css through StyleGuide.php;
                 the color groups come from resources/css/styleguide-colors.json, which
                 that skill writes from the Figma Color board. --}}
      ...
      <section class="sg-section" id="spacing" aria-labelledby="spacing-title">
        ...
        <div class="sg-grid sg-grid-swatches">
          @foreach ($shadowTokens as $token)
            <div class="sg-tile">
              <div class="sg-swatch"
                style="background-color: var(--color-paper); box-shadow: var({{ $token['token'] }});"></div>
        ...
        <div class="sg-grid">
          <div class="sg-tile">
            <div class="sg-specimen" style="display: flex; flex-wrap: wrap; gap: 1rem; align-items: center;">
              ...
              <a class="btn btn-link" href="#buttons">{{ __('Link', '__TEXT_DOMAIN__') }}</a>
              <button class="btn btn-primary" type="button" disabled>{{ __('Disabled', '__TEXT_DOMAIN__') }}</button>
            </div>
            <p class="sg-label">.btn-primary · .btn-secondary · .btn-link · :disabled</p>
          </div>
          <div class="sg-tile sg-tile-dark">
            <div class="sg-specimen" style="display: flex; flex-wrap: wrap; gap: 1rem; align-items: center;">
              <a class="btn btn-on-dark" href="#buttons">{{ __('On dark', '__TEXT_DOMAIN__') }}</a>
              <a class="btn btn-link" href="#buttons">{{ __('Link on dark', '__TEXT_DOMAIN__') }}</a>
            </div>
            <p class="sg-label" style="color: var(--color-muted-on-dark);">.btn-on-dark · .btn-link</p>
          </div>
        </div>
      </section>
      ...
      <section class="sg-section" id="components" aria-labelledby="components-title">
        ...
        <x-alert type="error">{{ __('Error alert.', '__TEXT_DOMAIN__') }}</x-alert>
      </section>
    </div>
  </div>
@endsection
