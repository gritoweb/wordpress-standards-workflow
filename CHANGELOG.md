...
top of it, rewritten on our token names (`docs/merge-decisions.md`).

### Added
- **Contract tokens:** `--color-link`, `--color-link-hover`, `--color-focus`,
  `--color-placeholder`, and the dark-ground twins `--color-muted-on-dark`,
  `--color-link-on-dark`, `--color-link-hover-on-dark`,
  `--color-focus-on-dark` (the wizard derives them when the guide doesn't
  give them). No existing name changed.
- **Buttons:** `.btn-on-dark` and `.btn-link`, plus focus ring, disabled,
  loading (spinner) and the icon slot `ActionEditor` already printed
  (`btn-icon-arrow|external|download`), in the wizard's `button.css`.
- **Grounds:** `global/grounds.css`, generated by `kit-setup` from
  `kit.config.json`, imported by `app.css`/`editor.css`.
- **Contrast gate:** `scripts/contrast.mjs` (WCAG AA, `asDrawn` for a failure
  the design draws on purpose; CLI prints one line per failing pair) and the
  wizard's `contrast-pairs.json`.
- **Kit CSS on our tokens:** `components/forms.css`, `selection.css`,
  `forms-gravity.css`, `social-icons.css`, `page-header.css`,
  `pages/archive.css`, and the icon set in `resources/images/icons/`.
- **Style guide page** (`template-styleguide.blade.php` + `StyleGuide`
  composer), reading our three token files.
- **`figma-design-system` skill:** builds the wizard's files from a Figma
  file, phase by phase; its foundation and components references point at the
  wizard's contract instead of their own.
- Kit tests: the wizard's example tokens define everything the check requires
  and pass its own contrast pairs; a theme built from `theme/` plus the
  wizard's code blocks passes `kit-setup`, `check-css-foundation` and
  `contrast.mjs`.

### Changed
- `check-css-foundation` requires the new tokens, `.btn-on-dark`/`.btn-link`,
  the `grounds.css` import and `contrast-pairs.json`.
- `base.css` links read `--color-link`/`--color-link-hover`; `:focus-visible`
  draws `--color-focus`.

### Verified
- `npm test` exit 0. The install test fails when a kit CSS file writes a raw
  color (checked by adding one, then reverting). The first draft's placeholder
  default measured 3.88:1 and failed the gate; it is now `var(--color-muted)`.
